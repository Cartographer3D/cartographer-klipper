    ...
    if [ -d ~/katapult ]; then
      ...
      if [[ $canCheck != "" ]]; then
        ...
        if [[ $canbootCheck != "" ]]; then
          ...
          canbootID=$(
            ~/klippy-env/bin/python ~/klipper/scripts/canbus_query.py can0 | grep -v 'Klipper$' | grep -v 'Total.*uuids found' | grep 'CanBoot' | awk -F'canbus_uuid=' '{print $2}' | awk -F', ' '{print $1}'
          )
          found=1
        fi
        ...
        if [[ $katapultCheck != "" ]]; then
          ...
          katapultID=$(
            ~/klippy-env/bin/python ~/klipper/scripts/canbus_query.py can0 | grep -v 'Klipper$' | grep -v 'Total.*uuids found' | grep 'Katapult' | awk -F'canbus_uuid=' '{print $2}' | awk -F', ' '{print $1}'
          )
          found=1
        fi
      fi
