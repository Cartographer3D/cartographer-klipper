    ...
    def cmd_SCANNER_TOUCH(self, gcmd):
        ...
        if gcmd.get("METHOD", "None").lower() == "manual":
            ...
            if success:
                ...
                self.log_debug_info(
                    verbose, gcmd, f"Standard Deviation: {standard_deviation:.4f}"
                )
                if calibrate == 1:
...
class ScannerMeshHelper:
    ...
    def _sample_mesh(self, gcmd, path, speed, runs):
        ...
        def cb(sample):
            ...
            if zcs > 0:
                # TODO: We should ensure these are not None
                dx = x - self.zero_ref_mode[1][0]  # pyright: ignore [reportOptionalSubscript]
                dy = y - self.zero_ref_mode[1][1]  # pyright: ignore [reportOptionalSubscript]
                dist = math.sqrt(dx * dx + dy * dy)
                if dist <= zcs:
    ...
    def _load_interpolator(self):
        ...
        if hasattr(self.scipy.interpolate, "RBFInterpolator"):
            ...
            def rbf_interp(points, values, faulty):
                # TODO: scipy can in theory be set to None after this has been declared..
