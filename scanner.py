...
class Scanner:
    ...
    def _check_hardware(self, sample):
        """
        Validates probe sample for faults like shorted coils or invalid frequencies.
        """
        if self._stream_en == 0:
            return
        
        freq = sample.get("freq")
        if freq is not None and self.fmin is not None:
            if freq > self.fmin * 1.35:
                if self._stream_en:
                    raise self._mcu.error(f"Frequency exceeded expected range. Potential coil issue. Freq: {freq:.2f}")
                self.gcode.respond_raw("Frequency exceeded expected range. Potential coil issue.")
            if sample.get("data") == SHORTED_COIL_VALUE:
                if self._stream_en:
                    raise self._mcu.error("Coil is shorted or not connected.")
                self.gcode.respond_raw("Coil is shorted or not connected.")
