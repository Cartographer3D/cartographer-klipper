...
headers_to_comment = ["scanner", "cartographer", "adxl345", "resonance_tester", "lis2dw", "bed_mesh", "probe", "temperature_sensor Cartographer_MCU"]
        ...
        for header, pattern in header_patterns.items():
            if pattern.match(line):
                ...
                if not line.strip().startswith("#"):
                    ...
                    updated_lines.append(line)  # Keep the line as is if already commented
                break
        else:
            if any(in_section.values()):
                if section_header_pattern.match(line):
                    ...
                    if not line.strip().startswith("#"):
                        updated_lines.append("#" + line)  # Commenting out the line if not already commented
                        lines_commented = True
                    else:
                        updated_lines.append(line)  # Keep the line as is if already commented
            else:
...
def get_position_max():
    ...
    if debug:
        ...
        print(f"  Adjusted values: x_adjusted: {position_max['x'] - 50}, y_adjusted: {position_max['y'] - 50}")
        print("="*60)
            ...
            if stripped_line.startswith("[stepper_z]"):
                ...
                if inside_stepper_z:
                    ...
                    if "endstop_pin:" in stripped_line and not endstop_pin_updated:
                        ...
                        updated_lines.append("homing_retract_dist: 0 # cartographer needs this to be set to 0\n")
                        homing_retract_updated = True
                        debug_print(f"Updated homing_retract_dist line.")  # Debugging output
                        ...
                        if not stripped_line.startswith("#"):
                            ...
                            debug_print(f"Commented out position_endstop line.")  # Debugging output
                        else:
...
def find_uuid_lines_in_log(canbus_uuids):
    if debug:
        ...
        print("="*60 + "\n")
        print(f"Searching for canbus_uuid: {canbus_uuids} in log file: {klippy_log_path}")
            ...
            for i, line in enumerate(log_lines):
                ...
                for uuid in canbus_uuids:
                    if uuid in line:
                        ...
                        last_sections = section_headers[-3:]  # Get the last three section headers

                        # Check if any of the last sections are "scanner" or "cartographer"
                        is_relevant_section = any("scanner" in section.lower() or "cartographer" in section.lower() for section in last_sections)

                        # Only add the line if it's under a relevant section
                        if is_relevant_section:
                            colored_line = f"\033[32m{line.strip()}\033[0m"  # Color the line green
                            found_lines.append((colored_line, last_sections))
                            break  # No need to check other UUIDs for this line

            if debug:
                if found_lines:
                    print(f"Found {len(found_lines)} relevant lines with UUIDs in the log.")
                else:
                    print("No relevant lines found with the specified canbus_uuids in the log.")
